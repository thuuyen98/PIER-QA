...
def get_table_surya(page, table_box, space_tol=.01) -> List[List[str]]:
    ...
    for block_idx, block in enumerate(sorted_blocks):
        ...
        for line_idx, line in enumerate(sorted_lines):
            ...
            cells = [[s.bbox, s.text.strip(' ')] for s in line.spans]
            ...
            for span in line["spans"]:
                for char in span["chars"]:
                    ...
                    # Use x_start < new_line_start_x to account for out-of-order cells in the pdf
                        ...
                        if len(table_cell) > 0:
                            table_row.append((cell_bbox, cell_content.strip()))
                        table_cell = char["char"]
                        cell_bbox = char["bbox"]
                        if len(table_row) > 0:
...
def format_tables(pages: List[Page]):
    ...
    for page in pages:
        ...
        for table_idx, table_box in enumerate(page_table_boxes):
            ...
            # table_text = tabulate(table_rows, headers="firstrow", tablefmt="html", disable_numparse=True)
            table_text = f"[table_{table_count}]{pd.DataFrame(table_rows).to_dict('records')}[/table_{table_count}]"
            table_block = Block(
                bbox=table_box,
                block_type="Table",
                pnum=pnum,
